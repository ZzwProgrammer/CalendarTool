var chrono = (() => {
  ...
  var SINGLE_TIME_UNIT_NO_ABBR_PATTERN3 = `(${NUMBER_PATTERN10})\\s{0,5}(${matchAnyPattern(TIME_UNIT_NO_ABBR_DICTIONARY2)})\\s{0,5}`;
  ...
  var PATTERN62 = new RegExp(`(?:(?:\\,|\\(|\\\uFF08)\\s*)?(?:(viime|edellinen|edellisen\xE4|ensi|seuraava|seuraavana|t\xE4m\xE4|t\xE4n\xE4)\\s*)?(${matchAnyPattern(WEEKDAY_DICTIONARY11)})(?:\\s*(?:\\,|\\)|\\\uFF09))?(?:\\s*(viime|ensi|seuraava)\\s*viikolla)?(?=\\W|$)`, "i");
  ...
  var PATTERN63 = new RegExp(`([0-9]{1,2})\\.?(?:\\s*(?:\\-|\\\u2013|\\s)\\s*([0-9]{1,2})\\.?)?\\s*(${matchAnyPattern(MONTH_DICTIONARY11)})(?:(?:-|/|,?\\s*)([0-9]{4}(?![^\\s]\\d)))?(?=\\W|$)`, "i");
  var DATE_GROUP16 = 1;
  ...
  var FIMonthNameLittleEndianParser = class extends AbstractParserWithWordBoundaryChecking {
    ...
    innerExtract(context, match) {
      const result = context.createParsingResult(match.index, match[0]);
      const month = MONTH_DICTIONARY11[match[MONTH_NAME_GROUP21].toLowerCase()];
      ...
      result.start.assign("month", month);
      result.start.assign("day", day);
      if (match[YEAR_GROUP26]) {
        const yearNumber = parseYear10(match[YEAR_GROUP26]);
        result.start.assign("year", yearNumber);
      } else {
        const year3 = findYearClosestToRef(context.refDate, day, month);
        result.start.imply("year", year3);
  ...
  var PATTERN64 = new RegExp(`(seuraava|seuraavat|seuraavien|edellinen|edelliset|edellisten|viimeiset|viimeisten|kuluneet|kuluneiden|\\+|-)\\s*(${TIME_UNITS_PATTERN10})(?=\\W|$)`, "i");
  var PATTERN_NO_ABBR3 = new RegExp(`(seuraava|seuraavat|seuraavien|edellinen|edelliset|edellisten|viimeiset|viimeisten|kuluneet|kuluneiden|\\+|-)\\s*(${TIME_UNITS_NO_ABBR_PATTERN3})(?=\\W|$)`, "i");
  var FITimeUnitCasualRelativeFormatParser = class extends AbstractParserWithWordBoundaryChecking {
    ...
    innerPattern() {
      return this.allowAbbreviations ? PATTERN64 : PATTERN_NO_ABBR3;
    }
    innerExtract(context, match) {
      const prefix = match[1].toLowerCase();
  ...
  var PATTERN65 = new RegExp(`(nyt|t\xE4n\xE4\xE4n|huomenna|ylihuomenna|eilen|toissap\xE4iv\xE4n\xE4|viime\\s*y\xF6n\xE4)(?:\\s*(aamulla|aamuna|aamup\xE4iv\xE4ll\xE4|p\xE4iv\xE4ll\xE4|iltap\xE4iv\xE4ll\xE4|illalla|y\xF6ll\xE4|keskiy\xF6ll\xE4))?(?=\\W|$)`, "i");
  var DATE_GROUP17 = 1;
  var TIME_GROUP3 = 2;
  ...
  // node_modules/chrono-node/dist/esm/locales/fi/index.js
  var casual15 = new Chrono(createCasualConfiguration14());
  var strict15 = new Chrono(createConfiguration14(true));
  ...
  __export(vi_exports, {
    ...
    Weekday: () => Weekday,
    casual: () => casual16,
    createCasualConfiguration: () => createCasualConfiguration15,
    createConfiguration: () => createConfiguration15,
    parse: () => parse16,
  ...
  var YEAR_PATTERN10 = "(?:[0-9]{1,4}(?:\\s*TCN)?)";
  ...
  var PATTERN66 = new RegExp("(?:ng\xE0y\\s*)?([0-9]{1,2})\\s*th\xE1ng\\s*([0-9]{1,2})(?:\\s*n\u0103m\\s*(" + YEAR_PATTERN10 + "))?(?=\\W|$)", "i");
  var DAY_GROUP6 = 1;
  var MONTH_GROUP8 = 2;
  var YEAR_GROUP27 = 3;
  var VIStandardParser = class extends AbstractParserWithWordBoundaryChecking {
    ...
    innerExtract(context, match) {
      ...
      if (match[YEAR_GROUP27]) {
        result.start.assign("year", parseYear11(match[YEAR_GROUP27]));
      } else {
  ...
  var PATTERN67 = new RegExp("(" + matchAnyPattern(MONTH_DICTIONARY12) + ")(?:\\s*(?:n\u0103m|/)\\s*(" + YEAR_PATTERN10 + "))?(?=\\W|$)", "i");
  var MONTH_GROUP9 = 1;
  var YEAR_GROUP28 = 2;
  var VIMonthYearParser = class extends AbstractParserWithWordBoundaryChecking {
    ...
    innerExtract(context, match) {
      ...
      if (match[YEAR_GROUP28]) {
        result.start.assign("year", parseYear11(match[YEAR_GROUP28]));
      } else {
  ...
  var PATTERN68 = new RegExp("(?:\\bn\u0103m\\s*(" + YEAR_PATTERN10 + ")|\\b([0-9]{1,4})\\s*(TCN))(?=\\W|$)", "i");
  ...
  var PATTERN69 = /\b(hôm nay|hôm qua|hôm kia|ngày mai|ngày kia|bây giờ|lúc này)(?=\W|$)/i;
  ...
  var PATTERN70 = /(buổi\s*)?(sáng sớm|sáng|trưa|chiều|tối|đêm|nửa đêm|bình minh)(?=\W|$)/i;
  ...
  var PATTERN71 = new RegExp("(" + matchAnyPattern(WEEKDAY_DICTIONARY12) + ")(?:\\s*(n\xE0y|t\u1EDBi|sau(?!\\s*khi)|qua))?(?=\\W|$)", "i");
  ...
  var PATTERN72 = new RegExp("(?:l\xFAc\\s*|v\xE0o\\s*)?([0-9]{1,2})(?:\\s*gi\u1EDD\\s*([0-9]{1,2})?\\s*(?:ph\xFAt\\s*)?(s\xE1ng|tr\u01B0a|chi\u1EC1u|t\u1ED1i|\u0111\xEAm)?|:([0-9]{2}))(?=\\W|$)", "i");
  var HOUR_GROUP7 = 1;
  ...
  var PATTERN73 = new RegExp("(" + TIME_UNITS_PATTERN11 + ")\\s{0,5}(?:tr\u01B0\u1EDBc|qua)(?=\\W|$)", "i");
  ...
  var PATTERN74 = new RegExp("(" + TIME_UNITS_PATTERN11 + ")\\s{0,5}(?:sau|n\u1EEFa|t\u1EDBi|ti\u1EBFp)(?=\\W|$)", "i");
  ...
  var PATTERN75 = new RegExp("(?:trong\\s*(?:v\xF2ng\\s*)?)(" + TIME_UNITS_PATTERN11 + ")(?=\\W|$)", "i");
  ...
  var PATTERN76 = new RegExp("(n\xE0y|tr\u01B0\u1EDBc|qua|sau|t\u1EDBi|ti\u1EBFp)\\s*(" + CASUAL_UNIT_PATTERN + ")|(" + CASUAL_UNIT_PATTERN + ")\\s*(n\xE0y|tr\u01B0\u1EDBc|qua|sau|t\u1EDBi|ti\u1EBFp)(?=\\W|$)", "i");
  ...
  // node_modules/chrono-node/dist/esm/locales/vi/index.js
  var casual16 = new Chrono(createCasualConfiguration15());
  var strict16 = new Chrono(createConfiguration15(true));
  ...
  function createCasualConfiguration15(littleEndian = true) {
    const option = createConfiguration15(false, littleEndian);
  ...
  function createConfiguration15(strictMode = true, littleEndian = true) {
    return includeCommonConfiguration({
      parsers: [
        new ISOFormatParser(),
        new SlashDateFormatParser(littleEndian),
  ...
  // js/vendor/chrono-entry.js
  console.log("[chrono] Available locales:", Object.keys(esm_exports).filter((k) => !k.startsWith("_") && typeof esm_exports[k] === "object"));
  window.chrono = esm_exports;
})();
